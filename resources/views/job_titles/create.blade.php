            ...
            <form method="POST" action="{{ route('job_titles.store') }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="nombre" name="nombre" required 
                           ...
                           value="{{ old('nombre') }}" placeholder="Ej: Ingeniero de Sistemas">
                    @error('nombre')
                ...
                <div class="mb-6">
                    ...
                    <input type="number" id="salario_base" name="salario_base" required 
                           class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full"
                           value="{{ old('salario_base', 0) }}" min="0" step="1000" placeholder="Ej: 2500000">
                    <p class="text-xs text-gray-500 mt-1">Este es un valor de referencia, no fija el sueldo del empleado.</p>
                    @error('salario_base')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
